...
<head>
    ...
    <title>Blog - HangarHero.com</title>
    ...
    <nav class="navbar navbar-default">
      ...
      <div class="container">
        <div id="page-wrapper">
          {{range $key, $val := .posts}}
            <h1>{{$val.Title}}</h1>
            <span><h2>{{$val.Author}}, {{$val.PostDate}}</h2></span>
            <p class="lead">{{$val.Headline}}</p>
            <a href="/blogpost/{{$val.Id}}">Read More ----></a>
            <br>
          {{end}}
          <ul class="pagination">
            {{if .prev}}
              <li class="disabled"><a href="/blog/{{.minusone}}">&laquo;</a></li>
            {{end}}
            {{range $key, $val := .posts}}
              {{if eq $val .jumps}}
                <li class="active"><a href="/blog/{{$val}}">{{$val}}</a></li>
              {{else}}
                <li><a href="/blog/{{$val}}">{{$val}}</a></li>
              {{end}}
            {{end}}
            {{if .next}}
              <li><a href="/blog/{{.plusone}}">&raquo;</a></li>
            {{end}}
          </ul>
        </div>
      </div>
